...
export default function ContextScopeBar() {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-[#13151b] border border-border-subtle px-4 py-2 rounded-2xl max-w-4xl w-full gap-2">
      <div className="flex items-center gap-3 flex-wrap">
        <span className="text-[11px] font-mono uppercase tracking-wider text-on-surface-variant">
          Context Scope
        </span>
        <div className="flex items-center gap-2 bg-[#1a1d24] border border-border-subtle px-3 py-1 rounded-full text-xs">
          <span className="w-1.5 h-1.5 bg-secondary rounded-full" />
          <span className="text-white font-medium">Active Intelligence Context: {activeTopic}</span>
        ...
        {isPivoting ? (
          <form onSubmit={handlePivotSubmit} className="flex items-center gap-1.5">
            <input
              ...
              className="bg-[#090a0f] border border-border-subtle px-2 py-0.5 text-xs text-white rounded-md focus:outline-none focus:border-white"
            />
            <button
              type="submit"
              className="text-xs px-2 py-0.5 bg-white text-black rounded font-medium"
            >
              Set
            </button>
            <button
          ...
          <>
            <button
              ...
              className="text-xs text-on-surface-variant hover:text-white transition-colors flex items-center gap-1.5 font-medium px-2 py-1 rounded-lg"
              type="button"
            >
