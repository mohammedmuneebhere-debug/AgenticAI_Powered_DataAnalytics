        ...
        <div className="w-full h-44 bg-surface-lowest border border-surface-border/70 rounded-xl relative flex items-center justify-center overflow-hidden">
          <svg className="w-full h-full" viewBox="0 0 400 160">
            {positions.slice(1).map((position, index) => (
              <line key={`edge-${index}`} x1={positions[0].x} y1={positions[0].y} x2={position.x} y2={position.y} stroke="var(--border)" strokeWidth="1.5" />
            ))}
            {positions.map(({ node, x, y }, index) => (
              <g key={node.id}>
                <circle cx={x} cy={y} r={index === 0 ? 30 : 25} fill={index === 0 ? "var(--bg-elevated)" : "var(--bg-surface)"} stroke={index === 0 ? "#ffffff" : ["#34D399", "#FBBF24", "var(--text-muted)"][index % 3]} strokeWidth={index === 0 ? 2 : 1.5} />
                <text x={x} y={y - 3} fill="var(--text-primary)" fontSize="8" fontWeight="700" textAnchor="middle">{node.label.slice(0, 16).toUpperCase()}</text>
                <text x={x} y={y + 10} fill="var(--text-secondary)" fontSize="7" textAnchor="middle">{node.inf}</text>
              </g>
            ))}
          </svg>
        </div>
