        ...
        <div className="w-full h-56 relative bg-surface-lowest border border-surface-border/70 rounded-xl p-3 flex flex-col justify-between overflow-hidden">
          ...
          <svg className="w-full h-full relative z-10" preserveAspectRatio="none" viewBox="0 0 700 180">
            {pointCoordinates.map((point, index) => (
              <rect key={`bar-${index}`} x={point.x - 5} y={point.y} width="10" height={160 - point.y} rx="3" fill={index === pointCoordinates.length - 1 ? "var(--border)" : "var(--bg-elevated)"} />
            ))}
            {linePoints && <polyline fill="none" stroke="var(--text-secondary)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" points={linePoints} />}
            {pointCoordinates.length > 0 && (
              <>
                <line x1={pointCoordinates[pointCoordinates.length - 1].x} y1="180" x2={pointCoordinates[pointCoordinates.length - 1].x} y2={pointCoordinates[pointCoordinates.length - 1].y} stroke="var(--text-secondary)" strokeWidth="1.5" strokeDasharray="3,3" />
                <circle cx={pointCoordinates[pointCoordinates.length - 1].x} cy={pointCoordinates[pointCoordinates.length - 1].y} r="4.5" fill="var(--bg-surface)" stroke="var(--text-primary)" strokeWidth="2" />
              </>
            )}
          </svg>
